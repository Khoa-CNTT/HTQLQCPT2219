...
const OrderDetailPage = () => {
    ...
    return (
        <div className={styles.orderDetailsWrapper}>
            <h2 className={styles.orderDetailsTitle}>Thông tin đơn hàng:</h2>
            {/*<h2 className={styles.userName}>Xin chào, {userName}!</h2>*/}
            {orderDetails.length === 0 ? (
                <div className={styles.orderDetailsEmptyWrapper}>
                    <p className={styles.orderDetailsEmpty}>Không có đơn hàng nào được đặt.</p>
                    <button
                        className={styles.backToUserButton}
                        onClick={() => window.location.href = '/user'} // hoặc dùng useNavigate nếu bạn dùng hook
                    >
                        Quay lại
                    </button>
                </div>
